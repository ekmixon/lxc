...
def child_get(prev, path):
    return next((p['children'] for p in prev if p['path'] == path), None)
...
def add_allow(path):
    ...
    for b in blocks:
        ...
        if path[:l] == b['path']:
            ...
            break
    if found is None:
        print(f"allow with no previous block at {path}")
        sys.exit(1)
    p = path[l:].strip()
    while p[:1] == "/":
        p = p[1:]
...
def gen_denies(pathsofar, children):
    for c in children:
        for char in range(len(c['path'])):
            ...
            if (
                char == len(c['path']) - 2
                and c['path'][char : char + 2] == '**'
            ):
                continue
            x = collect_chars(children, c['path'], char)
            newdeny = ("deny %s/%s[^%s]*{,/**}" % (pathsofar, c['path'][:char], x))
            append_deny(newdeny)
        if c['path'] != '**':
            if c['path'][len(c['path']) - 1] != '*':
                newdeny = "deny %s/%s?*{,/**}" % (pathsofar, c['path'])
            else:
                newdeny = f"deny {pathsofar}/{c['path']}/**"
            append_deny(newdeny)
        if len(c['children']) != 0:
            newpath = f"{pathsofar}/{c['path']}"
